...
向导[1-6]:config(有则确认/无则首次配置引导并保存) → 学号密码 → 日期范围
        ...
        → 提交节奏(批大小/篇间隔/批间停顿)
→ 登录(失败立即退出,绝不重试) → fetch_batches 解析全部实习批次(含起止日期) → 当月日历查重
...
4. 批次解析:真实表单 `<option selected="selected" value="63">`,`fetch_pc()` 正则 ✓（已演进:2026-08 改为 `fetch_batches()` 解析全部批次+起止日期,提交时按日期经 `pick_pc()` 自动选批次,无覆盖兜底当前批次;实测账号同时存在 70/71 两批次,边界日期映射正确）
